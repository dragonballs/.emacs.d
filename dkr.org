...
** Keybindings
#+srcname: keybinding-custom
#+begin_src keybinding-lisp
;;Set buffer switch key
(global-set-key "\C-x\C-b" 'bs-show) ;;or 'electric-buffer-list
(global-set-key "\M-p"  'bs-cycle-previous)
(global-set-key "\M-n"  'bs-cycle-next)
(global-set-key "\C-x\C-d"  'find-dired)
(global-set-key "\C-x\C-r"  'find-grep)
...
;; ibuffer
(global-set-key (kbd "C-x C-l") 'ibuffer)
...
;;Do not auto generate backup files
