...
0.   - Install Mac OS X's suite of Developer Tools. These came with your
   ...
   - Note your user name or the name of your computer. If you don't
     know either, open the Terminal.app and do one or both of,
...
#+end_src
      ...
      You will need to know your login name to activate the final
      customization file properly. You can use the system name as well
      (or instead) but it's optional.
...
2. Checkout a version of the starter kit using git -- if you're new to
   ...
   #+begin_src sh
     git clone http://github.com/kjhealy/emacs-starter-kit.git
   #+end_src
3. If they're already present, back up your =.emacs= and =.emacs.d/=
   ...
   resulting directory to =~/.emacs.d= [1]
...
5. build Org-mode
   ...
   make
   #+end_src
6. Finally, change the name of the starter kit's =kjhealy.org= to that
...
#+begin_src emacs-lisp 
  ...
  (add-to-list 'package-archives
               ...
               t)
  (add-to-list 'package-archives '("kjhealy" . "http://kieranhealy.org/packages/") t)
  (package-initialize)
